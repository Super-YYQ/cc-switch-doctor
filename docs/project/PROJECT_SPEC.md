...
本文不把任何固定的 CC Switch 版本号视为可靠事实。实现任务开始时，必须通过官方 GitHub Release API、默认分支源码和提交记录重新确认最新 Release、默认分支 HEAD、数据库 schema、Provider 结构和协议适配器；并把实际检查到的版本、提交 SHA、schema 指纹和 UTC 检查时间写入本项目兼容性清单。若不同来源显示不一致，以官方 GitHub API 和实际源码为准，不得继续使用文档中的旧快照。CC Switch 当前许可证也必须在预检中再次核对。
...
应用不得主动创建业务数据库、业务配置、历史记录、诊断日志、Key 缓存或浏览器业务存储。必须区分“应用业务持久化”和 Tauri/WebView2、Windows、杀毒软件可能产生的运行时缓存、崩溃记录或系统日志：前者严格禁止，后者无法作绝对零写入承诺。应尽量把 WebView2 数据目录指向本次会话临时目录，并在正常退出时 best-effort 清理；清理失败不得包含 Key、Provider 配置或诊断结果。
...
应用关闭时必须取消未完成请求，并 best-effort 清理应用可控内存中的敏感字符串。Rust 凭据必须优先使用 `secrecy` 和 `zeroize`，避免无必要克隆；但文档不得声称能够清除 HTTP/TLS 库、操作系统或崩溃转储中的所有副本。
...
- 便携版：ZIP 内含 Release 模式 `CC-Switch-Doctor.exe` 和说明文件；它依赖 Windows 10/11 通常已安装的 Microsoft Edge WebView2 Runtime，缺失时应给出明确错误说明；
- 安装版：仅构建 NSIS `CC-Switch-Doctor-vX.Y.Z-Windows-x64-setup.exe`，`tauri.conf.json` 的 bundle target 明确设为 `nsis`，避免额外 WiX/MSI 依赖；
- 默认按当前用户安装，不要求管理员权限；
- 不注册后台服务；
- 不设置开机启动；
- 不创建托盘常驻；
...
reqwest（显式启用 Windows native-tls/Schannel，关闭不需要的默认特性）
serde / serde_json
tokio
...
仓库必须提交 `.nvmrc`（或等价 Node 主版本声明）和 `rust-toolchain.toml`。AI 在首次成功构建时选择并记录实际可用的 Node LTS 主版本与 Rust toolchain；CI 使用同一版本，避免“本地能构建、Actions 因版本漂移失败”。
...
- 自启动；
- 深链协议注册；
- 任意 URL 打开器或系统浏览器启动能力。

Release 模式必须关闭 WebView DevTools，配置严格 CSP。因为所有外部网络请求均由 Rust 发出，WebView 的 `connect-src` 不应允许任意互联网域名。前端禁止 `dangerouslySetInnerHTML`，不得接受后端返回的未转义 HTML。
...
1. 实现前从 CC Switch 当前 `tauri.conf.json`/`app_store.rs` 确认应用 identifier、Tauri Store 实际落盘位置和 `app_config_dir_override` 格式，再只读解析该明确位置；不得猜测或递归扫描整个用户目录；
2. `%USERPROFILE%\.cc-switch\cc-switch.db`；
3. Windows 历史兼容路径：`HOME\.cc-switch\cc-switch.db`，仅在默认路径不存在时使用；
4. 仍找不到时，显示“选择 cc-switch.db”按钮。

手工选择只对当前进程有效，不保存路径。前端默认只显示“已连接”和数据库文件名，不显示包含 Windows 用户名的完整路径；完整路径仅保留在 Rust 内存中。
...
- Key 是 `${VAR}`、`$env:VAR`、占位符或无法解析的间接引用；
- Base URL 不是 `http`/`https`；
- URL 含 userinfo；
- 无法可靠判断静态 Key 是否属于第三方推理凭据。

前端只能提交 opaque provider ID、测试模式和有限选项；不得通过 Tauri command 接收前端传入的原始 Base URL、API Key、Authorization Header 或任意请求 Body。
...
1. **上游 API 可用性**：Doctor 直接调用供应商 API，能够真实验证；
2. **协议兼容性**：Doctor 能验证某个上游支持 Chat、Responses、Anthropic 或 Gemini 格式；
3. **CC Switch/CLI 端到端可用性**：Doctor 不启动本地路由和任何 CLI，因此只能依据 CC Switch 规则推断，不能声称已完成端到端验证。

例如 Codex 配置中 Chat Completions 成功、Responses 失败时，结论应为“上游可用，推断需要 CC Switch Local Routing；本工具未验证实际路由链路”。Claude Desktop、OpenClaw 等依赖 CC Switch 代理转换的场景同理。
...
现代 OpenAI 兼容接口优先使用 `max_completion_tokens`。仅当上游明确返回“不支持该字段”时，允许在同一候选请求中回退到 `max_tokens`；该字段回退计入最大尝试次数，不能同时无脑发送两次。
...
必须分别实现并测试四套结构，而不是把 OpenAI `tools` JSON 直接复用于所有协议：

- Chat Completions：`tools[].function` 与 `tool_calls`；
- Responses：Responses API 对应的 function tool 定义与 function call output item；
- Anthropic Messages：`tools[].input_schema` 与 `tool_use` content block；
- Gemini Native：`functionDeclarations` 与 functionCall part。

某协议尚未完成独立 payload/parser 时，UI 必须显示“不支持该协议的 Tool Calling 检测”，不能伪装为已测试。

### 12.8 `/models` 测试

模型发现候选为相对当前规范化 Base URL 的 `/models` 与 `/v1/models`，必须去重并保持同源。`GET /models` 类请求只作为模型发现和鉴权辅助，不作为最终可用性证明。
...
- 同一 host 最大总请求数默认 30；
- 同一 Provider 最大 12；
- 同一 Key+host 组合遇到明确 401/403/402 后，不再对其他模型重复认证失败请求；
- 同一 host 连续两次 429 后暂停该 host 的剩余任务；
- 多条 CC Switch 配置引用完全相同的 host、Key、协议和模型时，在内存中复用结果，避免重复消耗；用于去重的标识只存在本次进程内，不落盘、不显示。
...
证据边界：
- 已验证上游 Chat Completions；
- 未启动、未验证 CC Switch Local Routing；
- 未启动、未验证 Codex 客户端。
...
```json
{
  ...
  "verifiedAt": "ISO-8601",
  "ccSwitch": {
    "latestObservedRelease": "<preflight-discovered>",
    "verifiedReleases": ["<actually-tested-ranges>"],
    "baselineCommit": "<sha>",
    "schemaFingerprints": [
      {
...
1. 数据库 schema 指纹；
2. 数据库中可用的版本/迁移信息；
3. 从 CC Switch 当前官方源码确认的、明确且有限的安装元数据位置读取 Windows 文件版本（可选）；
4. 无法得到本地软件版本时显示“未知”，仍以 schema 指纹决定是否允许测试。

v0.1.0 禁止枚举运行中进程、读取进程路径或检查 Codex/Claude/OpenCode 进程。版本提醒不依赖进程扫描。
...
```text
CC Switch 本地版本：未知或检测值
CC Switch 官方最新：<GitHub API 返回>
Doctor 已验证范围：<manifest 返回>
状态：上游有新版本，当前兼容性尚未验证
...
- `schedule` 每天一次；
- `workflow_dispatch`；
- 显式声明最小权限：`contents: read`、`issues: write`；
- 查询 `farion1231/cc-switch` 最新 Release 和默认分支 HEAD；
- 和 `compatibility/manifest.json` 的 `latestObservedRelease`、`baselineCommit` 比较；
- 若 Release 未变化但 baseline 之后以下路径发生变化，也创建或更新兼容审查 Issue：`database/schema.rs`、`database/migration.rs`、`provider.rs`、`proxy/providers/`、`proxy/model_mapper.rs`、Provider 类型定义和 release notes；
- 发现变化时创建或更新一个带 `upstream-change` 标签的 Issue；标签不存在时工作流应创建标签或不依赖预置标签；
- Issue 包含版本、提交范围、受影响路径、Release 摘要链接和待检查清单；
- 不自动把新版本标记为 verified；
- 不自动发布 Doctor；
- 通过固定 Issue 标题或隐藏 marker 查找现有开放 Issue，避免每天重复创建。

### 16.5 Doctor 自身更新

v0.1.0 只提醒并提供“复制官方 Release 地址”，不自动打开浏览器、不调用系统 URL opener，也不做静默自动更新。
...
- 默认使用 Windows 系统信任存储；
- 明确测试并记录 reqwest 在 Windows 下的代理行为；至少支持标准 `HTTP_PROXY`、`HTTPS_PROXY`、`NO_PROXY` 环境变量；
- 可在当前会话临时输入一个显式 HTTP/HTTPS 代理，但不保存，代理凭据不得进入前端日志或结果；
- 不修改系统代理；
- 不启动 CC Switch Local Routing；
- 不默认把请求发往 `127.0.0.1:15721`；
- 仅允许 `http`/`https` URL，拒绝 `file:`、`ftp:`、自定义 scheme；
- 默认阻断常见云元数据地址（例如链路本地 metadata IP/host），但允许用户明确配置的 `localhost`/`127.0.0.1` 第三方服务；
...
`StartDiagnosisRequest` 只能包含本次扫描生成的 opaque provider IDs、模式、并发数和允许范围内的临时模型选择；不得包含 Key、原始 URL、认证头或任意 JSON Body。Rust 必须验证每个 ID 仍属于当前内存快照，避免前端伪造请求。
...
- 更新警告；
- Key 不出现在 DOM；
- Release CSP 不允许 WebView 直接访问任意互联网；
- 生产构建关闭 DevTools；
...
- 所有数据库 fixture 必须是人工构造的 synthetic 数据，禁止把用户真实导出 SQL 脱敏后提交到仓库；
...
- 产品截图或由前端 fixture/demo 状态生成的真实 UI 截图；若一次 Goal 环境无法可靠截取 Tauri 窗口，截图不阻塞 v0.1.0 Release，但不得使用与实际 UI 不一致的虚假营销图；
- 产品定位；
- 安全保证；
- 支持范围；
- 安装方法；
...
所有第三方/官方 Actions 必须固定到完整 commit SHA，并在行尾注释对应版本；Dependabot 负责后续更新。不得仅使用可漂移的 `@main`，安全敏感工作流也不应只固定到宽泛 major tag。
...
`workflow_dispatch` 只能提供“构建验证/重跑已有 tag”的显式模式，不得在未存在对应 tag 时从任意分支凭空创建正式版本。工作流必须幂等：相同 tag 重跑时更新或复用已有 Release，不创建重复 Release。
...
8. 生成 NSIS setup EXE；
9. 从 `src-tauri/target/release/` 取得 Release binary，验证其为非零 Windows PE 文件后打包便携 ZIP；
10. 对 Tauri 默认产物进行显式重命名，保证最终资产名严格符合本规格；
11. 生成覆盖 setup EXE 和 portable ZIP 的 `SHA256SUMS.txt`；
12. 创建非草稿 GitHub Release；
13. 上传全部资产；
14. Release body 包含安全边界、兼容基线、WebView2 Runtime 说明和未签名提示；
15. 最后验证 Release API 中资产存在、名称正确且大小大于零。

可以使用官方 `tauri-apps/tauri-action`，但必须固定到经过审查的完整 commit SHA；也可以使用等价的显式 PowerShell/`gh` 脚本。不得依赖浮动 `@v0` 引用。
...
v0.1.0 的强制交付路径是 **unsigned Release**：

- 构建和 Release 必须成功；
- Release 明确标注“未签名，Windows SmartScreen 可能提示”；
- 不伪造签名；
- 不生成自签名证书冒充可信发布者；
- 不要求任何证书 Secrets。

可信 Windows 签名作为 v0.2.0 之后的独立任务。未来接入 PFX、Azure Artifact Signing 或 SignPath 时，必须按所选方案实现证书导入、thumbprint/signCommand、时间戳和签名验证；不能仅因为存在 `WINDOWS_CERTIFICATE`/密码两个 Secrets 就声称已自动签名。
...
- [ ] 跨源 redirect 不携带 Key；
- [ ] 未知 schema 时停止测试；
- [ ] 应用没有业务持久化文件；WebView2 临时目录只含运行时数据且正常退出后 best-effort 清理；
- [ ] 没有遥测或自动崩溃上传。
...
- [ ] SHA256SUMS 存在；
- [ ] 资产大小大于零；
- [ ] Windows CI 对便携 EXE执行启动 smoke test（启动后进程保持存活或返回预期“未找到数据库”界面状态，再由测试脚本结束）；真实干净 VM 手工验证记录为推荐项，不作为无法自动证明的硬阻塞；
...
- 用户临时把“可信签名”提升为首版硬要求但没有可用签名服务/证书；
...
9. 实现协议 adapters；
10. 实现智能诊断 planner，并明确区分“上游已验证”和“CC Switch/CLI 端到端仅推断”；
11. 实现结果分类和脱敏；
12. 实现 UI；
13. 实现版本检查和上游监控；
...
Release 工作流显式使用 `contents: write`；上游监控工作流显式使用 `issues: write`
```
...
- 实现每日 upstream-watch GitHub Action，既监控 Release，也监控影响 schema/Provider/协议的关键源码路径；
- 结果必须区分“上游 API 已验证”和“CC Switch Local Routing/CLI 端到端仅推断”；
- 不实现自动配置修改、CLI 实测、进程枚举、系统浏览器启动和自动更新。
...
- 上传：
  ...
  3. SHA256SUMS.txt
- v0.1.0 固定发布 unsigned 版本，并在 Release 中明确 SmartScreen 与 WebView2 Runtime 提示；不要实现或伪造可信签名；
...
本版相对 v1.0 修正了以下会影响一次性交付成功率或安全准确性的事项：

1. 移除固定 CC Switch “最新版本”事实，强制以任务启动时官方 API/源码预检为准；
2. 明确 Tauri/WebView2 无法保证操作系统层面的绝对零写盘，安全承诺改为“零业务持久化”；
3. 明确 Doctor 只验证上游 HTTP，CC Switch Local Routing 和各 CLI 端到端状态只能推断；
4. 禁止运行进程枚举和系统浏览器 opener，避免与“无进程启动”红线冲突；
5. 增加 WebView CSP、Release DevTools 关闭和前端不得接收原始 URL/Key；
6. 修正现代 OpenAI token 参数兼容、`/v1/models`、协议独立 Tool Calling；
7. 增加 host/Provider/凭据组合的会话请求预算和结果去重；
8. 上游监控扩展为 Release + 关键源码路径变化，并补充 `issues: write` 权限；
9. Release 仅由 tag 正式触发、要求幂等、固定 Actions commit SHA、显式重命名资产；
10. v0.1.0 明确走 unsigned 发版，避免 AI 在没有证书时实现一个无法验证的半成品签名分支；
11. 便携版明确 WebView2 Runtime 依赖和 CI 启动 smoke test边界；
12. fixture 改为纯 synthetic 数据，禁止提交真实配置“脱敏版”。
...
CLI：绝不调用
登录态：绝不读取
存储：无业务持久化；WebView2/OS 运行时缓存 best-effort 临时化和清理
